...
The managed shell uses the installed runtime support copy at
`~/.local/lib/agy/termux/runtime/lib.sh`. Run `agy setup`, `agy update`, or the
installer's `support` action to refresh installed support files from a checkout.
